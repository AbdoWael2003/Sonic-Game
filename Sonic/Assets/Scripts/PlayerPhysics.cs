...
public class PlayerPhysics : MonoBehaviour
{
    ...
    public static bool isChased = false;
    ...
    private void Update()
    {
        ...
        if(map_manager.bossFight && Boss.health <= 0)
        {
            map_manager.bossFight = false;
            isChased = false;
            Boss.health = bossHealth.maxHealth;
            GetCurrentPlayer().linearVelocity = Vector3.zero;
            GetCurrentPlayer().angularVelocity = Vector3.zero;
            GetCurrentPlayer().MovePosition(map_manager.originalPosition);
        }
        ...
        if (Input.GetKeyDown(KeyCode.Space) && (map_manager.bossFight || (GetCurrentPlayer().transform.position.y <= 11 && GetCurrentPlayer().transform.position.y >= 0)))
        {
            Jump();

        }

        if (Input.GetKey(KeyCode.Z) && current_player_index == 0 && sonic.transform.position.y <= 6 && sonic.transform.position.y >= 1.2 && !map_manager.bossFight)
        {
    ...
    private void OnCollisionEnter(Collision collision)
    {
        Debug.Log($"Collison tag is {collision.gameObject.tag}" +
            $"button is pressed? {Input.GetKeyDown(KeyCode.Z)}" +
            $"currentPlayer {current_player_index}");


        //if(collision.gameObject.CompareTag("throwables") && Input.GetKeyDown(KeyCode.Z) && current_player_index == 1)
        //{
        //    GameObject attackObject = Instantiate(throwable, knuckles.transform.position, knuckles.transform.rotation);
        //    //Vector3 direction = collision.gameObject.transform.position - knuckles.transform.position;
        //    //Vector3 pushDirection = direction.normalized;
        //    StartCoroutine(Shoot(attackObject.GetComponent<Rigidbody>(), knuckles.transform));
        //}

        if (collision.gameObject.transform.parent != null && collision.gameObject.transform.parent.CompareTag("bossPoint") && GetCurrentPlayer().transform.position.z > map_manager.lastZBossPosition)
        {
            ...
            GetCurrentPlayer().MovePosition(bossStartPoint.transform.position); // start position of the boss map
    ...
    private void FixedUpdate()
    {
        ...
        Rigidbody currentPlayer = GetCurrentPlayer();

        if (currentPlayer.transform.position.y >= 60)
        {
            currentPlayer.linearVelocity = new Vector3(currentPlayer.linearVelocity.x, -5, currentPlayer.linearVelocity.z);
            currentPlayer.angularVelocity = new Vector3(currentPlayer.angularVelocity.x, -5, currentPlayer.angularVelocity.z);
        }
        ...
        if (moveDirection != Vector3.zero)
        {
            ...
            if (current_player_index == 0)
            {
                ...
                sonic.transform.rotation = Quaternion.RotateTowards(sonic.transform.rotation, targetRot, turnSpeed * Time.fixedDeltaTime);
            ...
            if (current_player_index == 1)
            {
                ...
                knuckles.transform.rotation = Quaternion.RotateTowards(knuckles.transform.rotation, targetRot, turnSpeed * Time.fixedDeltaTime);
            ...
            if (current_player_index == 2)
            {
                ...
                tails.transform.rotation = Quaternion.RotateTowards(tails.transform.rotation, targetRot, turnSpeed * Time.fixedDeltaTime);
        ...
        // Apply movement (boost & strafe as before)
        Vector3 moveVelocity = moveDirection * moveSpeed;
        if (current_player_index == 0 && sonicBoost)
        {
            moveVelocity *= 2f;
        }
        if (current_player_index == 0)
            rigid_body_sonic.linearVelocity = new Vector3(moveVelocity.x * 1.4f, rigid_body_sonic.linearVelocity.y, moveVelocity.z * 1.4f);
        if (current_player_index == 1)
            rigid_body_knuckles.linearVelocity = new Vector3(moveVelocity.x * 1.4f, rigid_body_knuckles.linearVelocity.y, moveVelocity.z * 1.4f);
        if (current_player_index == 2)
            rigid_body_tails.linearVelocity =  new Vector3(moveVelocity.x * 1.4f, rigid_body_tails.linearVelocity.y, moveVelocity.z * 1.4f);
    ...
    void SwitchCharacter()
    {
        ...
        Quaternion rotation = GetCurrentPlayerObject().transform.rotation;
        ...
        if(current_player_index == 2 && map_manager.bossFight)
        {
            current_player_index = 0;
        }
        ...
        GetCurrentPlayerObject().transform.rotation = rotation;
